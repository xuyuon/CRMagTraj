...
from diffrax import diffeqsolve, Dopri5, ODETerm, SaveAt, PIDController, Event, TextProgressMeter, RecursiveCheckpointAdjoint
...
class TrajectorySolver:
    ...
    """
    def __init__(self, field_model, particle, dD: Float, Dmax: Float, rmin: Float, rmax: Float):
    ...
    def run(self, x0: Array, v0: Array, coordinate_system: str = "cartesian"):
        ...
        if coordinate_system == "spherical":
            ...
            v_north, v_east, v_down = spherical_to_cartesian(mag, jnp.pi - theta_nadir, phi_nadir)
            
            vx, vy, vz = spherical_vec_to_cartesian_vec(theta, phi, -v_down, -v_north, v_east)
            v0 = jnp.array([vx, vy, vz])
            
        
        self.sol = diffeqsolve(
            self.term,
            solver,
            t0=0.0,
            t1=self.Dmax,
            ...
            args=(self.particle.get_charge()*C_SI/self.particle.get_energy()/self.particle.get_beta(), C_SI*self.particle.get_beta()),
            saveat=saveat,
